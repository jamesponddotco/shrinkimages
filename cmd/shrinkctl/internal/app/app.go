...
COMMANDS:
   start         start the server for the Shrink Images service
   stop          stop the server for the Shrink Images service
   generate-key  generate a new API key for the Shrink Images service
...
func Run() int {
	...
	case "stop":
		...
		return 0
	case "generate-key":
		if err := GenerateKeyAction(); err != nil {
			fmt.Fprintf(os.Stderr, "error: %s\n", err)
		}
