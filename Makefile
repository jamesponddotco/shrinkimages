...
doc: # Builds the manpage.
	$(SCDOC) <cmd/shrinkctl/doc/shrinkctl.1.scd >shrinkctl.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 shrinkctl $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 shrinkctl.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
...
clean: # Cleans cache files from tests and deletes any build output.
	$(RM) -f cover.out shrinkctl shrinkctl.1
